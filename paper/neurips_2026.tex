...
\begin{table*}[!t]
  ...
  \begin{tabular}{lrrrrrrr}
    ...
    \midrule
    Treeffuser-published & 0.670 & 1.242 & 0.40 & 0.117 & 0.044 & 0.023 & 208.55\,s \\
    Treeffuser-score$^+$ (ours) & \textbf{0.681} & 1.131 & \textbf{0.62} & 0.069 & \textbf{0.020} & \textbf{0.015} & 32.18\,s \\
    Treeffuser-FM (ours) & 0.680 & \textbf{1.117} & 0.46 & \textbf{0.064} & 0.028 & 0.024 & 10.06\,s \\
    \midrule
...
Table~\ref{tab:headline} reports cross-dataset means under the tuning/evaluation protocol. The score-side recipe lifts CRPSS from 0.670 to 0.681, raises PIT-uniformity pass rate from 0.40 to 0.62, reduces 90/95\% coverage error from 0.044/0.023 to 0.020/0.015, and cuts Treeffuser sample time 6.5$\times$. FM is close on CRPSS, has the best aggregate normalized CRPS among displayed rows, and is another 3.2$\times$ faster than score$^+$, but gives back part of score$^+$'s coverage advantage. The headline FM row uses the VP path because it is the fastest Treeffuser operating point; the tuned mechanism ablation identifies residualized linear FM as the CRPS-optimal FM corner, so we treat VP and linear as latency and CRPS endpoints rather than a single winning path (Table~\ref{tab:mechanism-ablation}). Per-dataset raw-CRPS winners span five families; a Treeffuser variant wins on 6/10 datasets, with the remaining wins split across deep ensembles, CARD-style diffusion, and quantile-regression LightGBM. The Treeffuser aggregate advantage is directional rather than conventionally significant: paired Wilcoxon tests give one-sided $p=0.080$ for score$^+$ over published and $p=0.065$ for FM over published; FM versus score$^+$ remains indistinguishable ($p=0.922$, two-sided). Because the external families are tuned on an equal finite-trial budget but with differing search spaces and per-trial costs (\S\ref{sec:disc}), the cross-family rows are informative about the tradeoff surface rather than definitive head-to-head rankings.

\paragraph{End-to-end timing.} Table~\ref{tab:headline} excludes fitting, but fit time is logged on the same runs, so Table~\ref{tab:timing} reports both. Two points follow. First, FM is the cheapest end-to-end Treeffuser row (24.0\,s fit$+$sample), ahead of score$^+$ (45.4\,s) and far ahead of the published SDE (221.7\,s), so the sampling speedup is not bought back at fit time. Second, near-zero sampling time does not imply lower end-to-end cost: quantile-regression LightGBM samples in 0.30\,s but fits 24--49 separate quantile models, so its total cost is 69.6\,s, and the deep ensemble is close to score$^+$ at 46.3\,s but still slower than FM. Fit times are still environment-specific and use library-default parallelism, so we read them as the same kind of practical engineering benchmark as the sampling timings.
...
\begin{table}[!t]
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    Treeffuser-published & 13.14 & 208.55 & 221.69 \\
    Treeffuser-score$^+$ (ours) & 13.26 & 32.18 & 45.43 \\
    Treeffuser-FM (ours) & 13.90 & 10.06 & 23.96 \\
    \midrule
...
\begin{table}[!h]
  ...
  \begin{tabular}{lrrrrrrr}
    ...
    \midrule
    Treeffuser-score$^+$ (ours)     & \textbf{0.681} & 1.131 & 4.057 & $-$0.214 & \textbf{0.028} & \textbf{0.015} & 32.18 \\
    Treeffuser-FM-fast (ours)       & 0.680 & \textbf{1.117} & 4.121 & \textbf{$-$0.218} & 0.034 & 0.024 & 10.06 \\
    Treeffuser-published            & 0.670 & 1.242 & 4.279 & 1.358 & 0.044 & 0.023 & 208.55 \\
...
\begin{table}[!h]
  ...
  \begin{tabular}{lrrrrrrrrrr}
    ...
    \midrule
    yacht              & FM\,$+$0.4\%      & 0.438 & 0.284 & \textbf{0.283} & 0.092 & \textbf{0.025} & 0.081 & 0.3 & 0.2 & \textbf{0.0} \\
    diabetes           & Sco$^+$\,$+$1.9\% & 35.682 & \textbf{34.025} & 34.660 & 0.079 & \textbf{0.029} & 0.038 & 2.0 & 0.3 & \textbf{0.0} \\
    energy             & FM\,$+$5.2\%      & 0.252 & 0.214 & \textbf{0.204} & 0.081 & 0.034 & \textbf{0.022} & 3.3 & 0.4 & \textbf{0.3} \\
    concrete           & Sco$^+$\,$+$1.5\% & 2.457 & \textbf{2.126} & 2.158 & 0.034 & 0.025 & \textbf{0.023} & 3.4 & 4.3 & \textbf{0.1} \\
    wine               & Pub\,$+$5.9\%     & \textbf{0.300} & 0.319 & 0.318 & 0.037 & \textbf{0.006} & 0.016 & 113.2 & 44.1 & \textbf{10.0} \\
    kin8nm             & FM\,$+$4.0\%      & 0.067 & 0.058 & \textbf{0.055} & 0.012 & \textbf{0.004} & 0.023 & 24.1 & 19.1 & \textbf{5.8} \\
    power              & FM\,$+$2.4\%      & 1.671 & 1.572 & \textbf{1.535} & 0.013 & \textbf{0.004} & 0.006 & 17.2 & \textbf{2.5} & 3.3 \\
    naval              & FM\,$+$3.2\%      & 0.000218 & 0.000220 & \textbf{0.000211} & 0.078 & 0.065 & \textbf{0.061} & 115.2 & 66.8 & \textbf{15.9} \\
    cali.\ housing     & Pub\,$+$3.6\%     & \textbf{0.197} & 0.204 & 0.206 & 0.010 & \textbf{0.005} & 0.007 & 415.3 & 49.2 & \textbf{9.7} \\
    protein            & Pub\,$+$2.7\%     & \textbf{1.722} & 1.769 & 1.796 & \textbf{0.005} & 0.005 & 0.007 & 1391.6 & 135.0 & \textbf{55.4} \\
    \bottomrule
  \end{tabular}
\end{table}

FM is the fastest Treeffuser sampler on 9/10 datasets and is especially valuable at scale: on \texttt{protein} the speedup is $25.1\times$ over \textsc{published} and $2.4\times$ over \textsc{score$^+$}; on \texttt{california\_housing} the corresponding speedups are $42.8\times$ and $5.1\times$. On $|\text{cE}|$@90, however, \textsc{score$^+$} leads on 6/10 datasets and FM on 3/10, which is why Table~\ref{tab:headline} separates normalized-CRPS and calibration conclusions. Under this evaluation protocol, the per-dataset CRPS ranking reproduces the headline pattern: FM has the best average normalized CRPS, score$^+$ is the more reliable calibration row, and the published SDE remains best on wine, california housing, and protein.
...
\section{Large-Dataset Stress Test: CT Slice Localization (Preliminary)}\label{app:large-data}

Table~\ref{tab:robustness-size} flags the largest datasets as the regime where the published SDE remains strongest, but with only two datasets above 10{,}000 training rows the headline suite cannot say why. As a targeted stress test we add \texttt{ct\_slices} (CT slice axial-location regression; 53{,}500 rows and 384 numeric features after dropping the patient identifier; OpenML data id 46300) under the same six-fold tuned protocol (48{,}150/5{,}350 split; fold-0 tuning, folds 1--5 evaluation). We keep this dataset out of the ten-dataset headline aggregates: it carries diagnostic-only variant rows that the other datasets lack, and this appendix reports an ablation in progress rather than a settled comparison. The dataset is nearly deterministic given the features---every tuned family reaches CRPSS $\geq 0.85$---so CRPS differences are decided at conditional scales around one percent of the marginal spread.

\begin{table}[!h]
  \centering
  \caption{Tuned \texttt{ct\_slices} rows under the headline protocol (five evaluation folds; mean $\pm$ sd for CRPS, fold means otherwise). The residualizer-off twins are tuned under the identical protocol. The score-flex row tunes the score-side recipe axes jointly over a superset space with 50 fold-0 trials (versus 25 for the frozen bundles) and is reported as a preliminary superset result, not a headline method. The deep ensemble, the strongest baseline on this dataset, is shown for context.}\label{tab:ct-slices-tuned}
  \scriptsize
  \begin{tabular}{lrrrrr}
    \toprule
    Row & CRPS & CRPSS & $|\text{cE}|$@90 & Fit (s) & Sample (s) \\
    \midrule
    Published Euler-50           & $0.164 \pm 0.012$ & 0.987 & 0.086 & \textbf{176} & 851 \\
    Score$^+$ PF-ODE-25          & $0.463 \pm 0.006$ & 0.964 & \textbf{0.018} & \textbf{175} & 880 \\
    Score$^+$ no-resid.\ PF-ODE-25 & $0.287 \pm 0.005$ & 0.978 & 0.025 & 306 & 3297 \\
    FM-VP ODE-5                  & $0.426 \pm 0.007$ & 0.967 & 0.033 & 320 & \textbf{457} \\
    FM-VP no-resid.\ ODE-5       & $1.877 \pm 0.054$ & 0.853 & 0.078 & 187 & 337 \\
    Score-flex Euler-50 (superset) & $\mathbf{0.130 \pm 0.004}$ & \textbf{0.990} & 0.084 & 939 & 3208 \\
    Deep ensemble                & $0.159 \pm 0.020$ & 0.988 & 0.076 & 619 & $<1$ \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:ct-slices-tuned} reverses the small/medium-data ordering: the published SDE is the sharpest Treeffuser row and only the deep ensemble matches it on CRPS, while score$^+$ and FM trail by $2.6$--$2.8\times$. The residualizer-off twins split the explanation. Removing the residualizer roughly halves the score$^+$ gap ($0.463 \to 0.287$), so the fixed residualizer-C is a genuine partial bottleneck for the score-side bundle at this scale; the same switch makes FM collapse ($0.426 \to 1.877$), so FM depends on residualization here. The coverage column shows the other side of the tradeoff: the published row buys its CRPS with the worst 90\% coverage error among the tuned Treeffuser rows, while the score$^+$ variants stay well calibrated, consistent with the operating-point reading in \S\ref{sec:disc}.
...
Table~\ref{tab:ct-slices-solver} rules the sampler out for the score arm and localizes the FM collapse. First, swapping the score$^+$ samplers---doubling the Heun PF-ODE steps, or substituting the published 50-step Euler SDE---moves CRPS by less than $0.01$ on both score$^+$ variants. Second, the published model wins even when handicapped to 15 Euler steps ($0.208$ versus $0.281$ for the best score$^+$ cell at any step count) and is essentially converged at its default 50 steps. Third, the FM no-residualizer collapse is roughly half solver discretization---steps recover $1.85 \to 0.98$---but it plateaus far above the residualized row, while residualized FM is already solver-converged at 5 steps. Taken together, the published advantage on this dataset is attributable to what the score model learns at training time---candidate factors are the uniform-$t$ noise-level allocation (log-uniform in $\sigma$, roughly $3\times$ more training mass below $\sigma = 0.05$ than the log-$\sigma$-normal draw of \S\ref{sec:score-side}), the noise- versus EDM-parameterized regression target, and the explicit log-$\sigma$ feature---not to the sampler, and only partly to the residualizer. Isolating which factor dominates is work in progress; an initial transfer diagnostic (switching the tuned no-residualizer score$^+$ configuration to uniform-$t$ without retuning) does not recover the gap, which cautions against attributing it to the $t$ distribution alone.

The ablation adds one calibration observation: sampling the same no-residualizer score$^+$ model with the stochastic Euler SDE degrades its 90\% coverage error from $0.021$ (PF-ODE) to $0.090$, so the calibration edge of the score$^+$ rows on this dataset is contributed by the deterministic PF-ODE sampler, not by the training recipe alone. Finally, fit cost qualifies the end-to-end timing claim of \S\ref{sec:headline} at this scale: FM fits in 320\,s versus 176\,s for the published row (the $k$-fold residualizer and the deeper tuned ensembles dominate), so FM's end-to-end advantage on small and medium data does not carry to the largest datasets.

A superset variant closes and reverses the gap. Rather than comparing frozen bundles, we expose the score-side recipe axes---score parameterization, noise features, $t$ sampling and its log-$\sigma$ prior, residualization, and histogram resolution---as jointly tunable dimensions over the shared LightGBM surface, with the stochastic-SDE-versus-PF-ODE sampler treated as a recipe-coupled axis selected on fold~0. Tuned under the same protocol (50 fold-0 trials, versus 25 for the frozen bundles), the SDE arm of this \texttt{score-flex} space reaches CRPS $0.130 \pm 0.004$ (Table~\ref{tab:ct-slices-tuned}), surpassing both the tuned published SDE ($0.164$) and the deep ensemble ($0.159$)---the first Treeffuser row to lead this dataset on CRPS. Two controls isolate the source of the gain. First, the space explicitly seeds and evaluates the published corner (noise prediction / \texttt{raw\_time} / uniform $t$ / residualizer off), so the tuner reaching a different optimum is a selected move, not an unreachable one: roughly thirty trials touch that corner and the search still departs it for the EDM corner. Second, decomposing the fold-0 selection isolates recipe from capacity. The published corner's own-space fold-0 CRPS of $0.145$ improves to $0.130$ once histogram bins and forest depth are unlocked inside the flex space (\texttt{max\_bin}\ $255\!\to\!1023$, deeper ensembles), whereas the EDM corner reaches $0.124$ at the same capacity; so about seventy percent of the gap over the tuned published baseline is capacity and binning that the published recipe could also adopt, and the remaining, capacity-matched, part---best EDM $0.124$ versus best noise-prediction $0.130$ across the fifty trials---is the score-side recipe itself. The reversal carries the earlier calibration caveat rather than escaping it: the flex SDE arm inherits the stochastic sampler's weaker central coverage ($|\text{cE}|$@90 $0.084$, matching the published row's $0.086$ rather than the PF-ODE score$^+$ rows' $0.018$), and its sampling cost tracks the EDM no-residualizer score path (Table~\ref{tab:ct-slices-solver}), not the choice of sampler. We report this as a preliminary superset result on a single dataset with a larger space and double the trial budget, not as a headline claim; it shows that a jointly tuned score-side recipe under the SDE sampler both contains and surpasses the published optimum here, and it localizes the earlier ``training-time recipe'' explanation to the EDM parameterization once capacity is matched.
