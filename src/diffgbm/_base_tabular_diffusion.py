...
class BaseTabularDiffusion(BaseEstimator, abc.ABC):
    ...
    def fit(
        ...
        if self.residualize != "off":
            if y_transformed.shape[0] < MIN_RESIDUALIZE_ROWS:
                # Residualization is on by default, so too little data must degrade to the
                # unresidualized model rather than refusing to fit at all.
                warnings.warn(
                    f"Skipping residualize={self.residualize!r}: cross-fitting needs at least "
                    f"{MIN_RESIDUALIZE_ROWS} training rows but got {y_transformed.shape[0]}.",
                    UserWarning,
                    stacklevel=2,
                )
            else:
