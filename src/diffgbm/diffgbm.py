...
from diffgbm._base_tabular_diffusion import BaseTabularDiffusion
from diffgbm._residualizer import ResidualizeMode
from diffgbm._score_models import LightGBMScoreModel
from diffgbm._score_models import LightGBMVelocityModel
from diffgbm._score_models import ScoreModel
from diffgbm._score_models import VelocityModel
from diffgbm.sde import DiffusionSDE
from diffgbm.sde import get_diffusion_sde

# Score-only defaults, named so the flow-matching path can tell "left at the default"
# from "deliberately set to something flow matching cannot honor".
_DEFAULT_SCORE_PARAMETERIZATION = "edm"
_DEFAULT_NOISE_FEATURES = "raw_time_log_std"

# Flow matching builds its features from the path itself, so it always uses the
# raw-time layout regardless of the score-side default.
_FLOW_MATCHING_NOISE_FEATURES = "raw_time"


class DiffGBM(BaseTabularDiffusion):
    def __init__(
        self,
        n_repeats: int = 30,
        ...
        score_parameterization: str = _DEFAULT_SCORE_PARAMETERIZATION,
        noise_features: str = _DEFAULT_NOISE_FEATURES,
        edm_sigma_data: float = 1.0,
        loss_weighting: str = "uniform",
        min_snr_gamma: float = 5.0,
        t_sampling: str = "log_sigma_normal",
        uniform_endpoint_fraction: float = 0.05,
        log_sigma_p_mean: float = -1.2,
        log_sigma_p_std: float = 1.2,
        residualize: ResidualizeMode = "mean",
        ...
        """
        Probabilistic tabular regression with gradient-boosted diffusion and flow matching.

        The defaults are the score-side recipe: EDM preconditioning, an explicit log-noise
        feature, log-sigma time sampling, and conditional-mean residualization. The published
        Treeffuser recipe is the explicit configuration `score_parameterization="noise"`,
        ...
        sde_name : str
            SDE: Name of the SDE to use. See `diffgbm.sde.get_diffusion_sde` for available SDEs.
        sde_initialize_from_data : bool
            SDE: Whether to initialize the SDE from the data. If `True`, the SDE hyperparameters are
            initialized with a heuristic based on the data (see `diffgbm.sde.initialize.py`).
        ...
        score_parameterization : str
            ...
            "noise", "x0", and "edm". Default "edm" applies EDM preconditioning, which keeps
            the regression target unit-scale across noise levels. "noise" recovers the
            published Treeffuser target. Ignored under `training_objective="flow_matching"`.
        noise_features : str
            Noise/time feature representation passed to LightGBM. Currently supported:
            "raw_time" and "raw_time_log_std". Default "raw_time_log_std" adds an explicit
            log-noise feature aligned with EDM scaling. Ignored under
            `training_objective="flow_matching"`, which always uses the raw-time layout.
        edm_sigma_data : float
            Data standard deviation used by the EDM preconditioning coefficients when
            `score_parameterization="edm"`. The default 1.0 matches the published Treeffuser
        ...
        loss_weighting : {"uniform", "min_snr"}
            ...
            matches the published Treeffuser behavior. "min_snr" applies the parameterization-
        ...
        t_sampling : {"uniform", "log_sigma_normal", "log_snr_normal"}
            Distribution used to draw the training-time `t` values. "uniform" matches the
            published Treeffuser behavior (with a small random anchor at t=1 controlled by
            `uniform_endpoint_fraction` under flow matching).
            "log_sigma_normal" (default) follows Karras et al. (EDM) by drawing
        ...
        residualize : {"off", "mean", "mean_scale"}
            Conditional residualization before score-model fitting. "mean" (default)
            subtracts a cross-fitted conditional mean, moving conditional location out of
            the regression target; "mean_scale" additionally divides by a cross-fitted
            conditional scale; "off" recovers the published Treeffuser behavior.
    ...
    def _warn_on_flow_matching_ignored_params(self) -> None:
        ...
        if self.score_parameterization != _DEFAULT_SCORE_PARAMETERIZATION:
            ignored_params.append("score_parameterization")
        if self.edm_sigma_data != 1.0:
            ignored_params.append("edm_sigma_data")
        if self.noise_features != _DEFAULT_NOISE_FEATURES:
            ignored_params.append("noise_features")
    ...
    def get_new_velocity_model(self) -> VelocityModel:
        velocity_model = LightGBMVelocityModel(
            ...
            noise_features=_FLOW_MATCHING_NOISE_FEATURES,
            t_sampling=self.t_sampling,
            log_sigma_p_mean=self.log_sigma_p_mean,
            log_sigma_p_std=self.log_sigma_p_std,
