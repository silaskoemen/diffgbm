...
from diffgbm._score_models import _fit_one_lgbm_model

ResidualizeMode = Literal["off", "mean", "mean_scale"]

# Fewest training rows that support cross-fitted residualization (two folds of forty
# rows). Below this the caller must fall back to an unresidualized fit. This is the
# mechanical minimum for a cross-fit rather than a quality threshold: on synthetic data
# residualization still improves CRPS at every size from 80 rows upward, so a higher
# floor would forgo a gain rather than avoid harm.
MIN_RESIDUALIZE_ROWS = 80
