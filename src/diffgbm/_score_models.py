...
from diffgbm._flow_matching import FlowPath
...
class NoiseParameterization(ScoreParameterization):
    """
    Published Treeffuser behavior: train LightGBM to predict the added negative noise and
...
class EDMParameterization(ScoreParameterization):
    ...
    With standardized targets, `sigma_data=1` is the natural default. The published Treeffuser
...
class RawTimeFeatureBuilder(NoiseFeatureBuilder):
    """
    Published Treeffuser feature layout: [perturbed_y, X, t].
    """
...
class RawTimeLogStdFeatureBuilder(NoiseFeatureBuilder):
    """
    Published Treeffuser feature layout with explicit noise scale: [perturbed_y, X, t, log_std(t)].
    """
...
class UniformLossWeighting(LossWeighting):
    """
    Sample weight 1 for every row. Reproduces the published Treeffuser behavior.
    """
...
class UniformTSampler(TSampler):
    """Uniform `t` on `[EPS, T]`. Reproduces the published Treeffuser behavior."""
...
class LogSigmaNormalTSampler(TSampler):
    ...
    p_mean, p_std : float
        ...
        image data on standardized targets; the published Treeffuser uses standardized targets too so
